...
| Fairness | Implementerad kärna, orkestrering och spelarverifiering | Deterministisk byte-stream, rejection sampling, shuffle och Node/Web Crypto-golden vectors finns. Application service äger seed, commitment, nonce, shuffle och pocket; privat commitment/seed/reveal persisteras atomiskt med rundan. Efter settlement kan spelaren kontrollera commitment och återskapa roulettepocket eller blackjackskons utdelade kortordning lokalt i webbläsaren. Det Zod-validerade rundtranskriptet bevaras lokalt per bord så att samma kontroll fungerar efter sidrefresh. |
...
| `GET /ready` | Implementerad och driftprobad | Returnerar 200 först när det privata Postgres-spelschemat är migrerat och committed-event-reläet lyssnar. Container-CI verifierar 503 mot en tom databas; databas-CI terminerar den riktiga LISTEN-sessionen och verifierar 503, socketnedkoppling, ny LISTEN i samma process och återgång till 200. |
...
| Socket.IO `server.ready` | Implementerad, kontrakterad och direkt testad | Bekräftar anslutning med ett strikt `ServerReadyV2`-payload som har fixture och genererad JSON Schema. Supabase-token verifieras vid handshake och omverifieras periodiskt; återkallad eller identitetsändrad session kopplas ned. |
...
| Spelarkonto | Implementerad lokalt, extern OAuth ej driftverifierad | `/konto` kan starta en gästsession, skapa och uppdatera spelarens ägarisolerade profil samt länka samma Supabase-identitet till Google. Webbrendering, responsivitet, lint, typkontroll och komponenttester är verifierade. Google provider, manual identity linking, redirect-allowlist och hosted migration måste aktiveras innan OAuth-flödet kan verifieras i preview/produktion. |
