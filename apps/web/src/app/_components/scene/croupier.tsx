...
export type CroupierVisualPose =
  | "rest"
  | "present"
  | "deal"
  | "reveal"
...
const POSES = {
  ...
  reveal: { lift: 0.09, reach: 0.18, wrist: -0.62, curl: 0.2 },
  celebrate: { lift: 0.16, reach: 0.02, wrist: -0.46, curl: 0.08 },
  sympathetic: { lift: -0.015, reach: 0.08, wrist: -0.28, curl: 0.4 },
} satisfies Record<CroupierVisualPose, Pose>;
...
function DealerArm({
  ...
  useFrame((state, delta) => {
    ...
    // The semantic event was already projected to this approved visual pose.
    // PoseMixer only cross-fades the geometry and never infers game state.
    mixer.play(poseName, 0.7);
