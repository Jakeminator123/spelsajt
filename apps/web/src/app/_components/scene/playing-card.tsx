...
export function PlayingCard({
  ...
  const face = useMemo(() => makeFaceTexture(rank, suit), [rank, suit]);
  const back = useMemo(() => makeBackTexture(), []);

  // Flip the card around Z when face-down so the back shows on top (+Y).
  const flip = faceUp ? 0 : Math.PI;

  return (
    <group position={position} rotation={[0, rotationY, flip]}>
